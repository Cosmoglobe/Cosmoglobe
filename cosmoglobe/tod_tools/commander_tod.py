...
class TODLoader:
    ...
    def init_file(self, freq, od, mode="r"):
        ...
        if not self.od and self.od !=0:
            ...
            self.outName = os.path.join(
                self.outPath, self.name + "_" + sfreq + "_" + str(od).zfill(6) + ".h5"
            )
            ...
            try:
                ...
                if self.exists and not self.overwrite:
                    for pid in self.load_field("/common/pids"):
                        loadBalance = self.load_field(
                            "/" + str(pid).zfill(6) + "/common/load"
                        )
                        self.spinaxis[pid] = (
                            str(float(loadBalance[0]))
                            + " "
                            + str(float(loadBalance[1]))
                        )
        ...
        if dims[0] != len(columnInfo):
            if dims[1] == len(columnInfo):
                ...
                raise ValueError(
                    "Data is shape "
                    + str(shape)
                    + " but column headers have length "
                    + str(len(columnInfo))
                )
            ...
            for compArr in compression:
                ...
                if compArr[0] == "dtype":
                    ...
                    raise ValueError(
                        "Compression type "
                        + compArr[0]
                        + " is not a recognized compression"
                    )
    ...
    def add_encoding(self, encoding, value):
        if encoding not in self.encodings.keys():
            ...
            if self.encodings[encoding] != value:
                print(
                    "Warning: Inconsistant encoding value "
                    + encoding
                    + " is set to "
                    + str(self.encodings[encoding])
                    + " but wants to be "
                    + str(value)
                )
    ...
    def finalize_file(self):
        ...
        if not self.exists or self.overwrite:
            ...
            self.add_field("/common/pids", list(self.spinaxis.keys()))

        if self.filelists is not None:
            for pid in self.spinaxis.keys():
                self.filelists[self.freq]["id" + str(pid)] = (
                    str(pid)
                    + ' "'
                    + os.path.abspath(self.outName)
                    + '" '
                    + "1 "
                    + self.spinaxis[pid]
                    + "\n"
    ...
    def finalize_chunk(self, pid, loadBalance=[0, 0]):
        ...
        for key in self.huffDict.keys():
            ...
            huffArray = np.append(
                np.append(np.array(h.node_max), h.left_nodes), h.right_nodes
            )
            numStr = str(key)
            if key == 1:
                numStr = ""
            ...
            self.add_field(
                "/" + str(pid).zfill(6) + "/common/hufftree" + numStr, huffArray
            )
            self.add_field(
                "/" + str(pid).zfill(6) + "/common/huffsymb" + numStr, h.symbols
            )
            ...
            for field in self.huffDict[key].keys():
                if ":" not in field:
                    self.add_field(
                        field, np.void(bytes(h.byteCode(self.huffDict[key][field])))
                    )
                    # print(self.huffDict[key][field], len(h.byteCode(self.huffDict[key][field])))
                else:
                    fieldName, index = field.rsplit(":", 1)
                    self.raggedDict[fieldName][index] = np.void(
                        bytes(h.byteCode(self.huffDict[key][field]))
                    )
        ...
        for field in self.raggedDict.keys():
            ...
            if ragged == False:
                ...
                array = np.ndarray(
                    (len(self.raggedDict[field].keys()), length), dtype=dataType
                )
                ...
                dset = self.outFile.create_dataset(
                    field, len(list(self.raggedDict[field].keys())), dtype=dt
                )
                for entry in self.raggedDict[field].keys():
                    dset[int(entry)] = np.frombuffer(
                        self.raggedDict[field][entry], dtype="uint8"
                    )
        ...
        self.spinaxis[pid] = str(float(loadBalance[0])) + " " + str(float(loadBalance[1]))
    ...
    def make_filelists(self):
        for freq in self.filelists.keys():
            outfile = open(
                os.path.join(self.outPath, "filelist_" + str(freq) + ".txt"), "w"
            )
        ...
        for i in range(ndim):
            ...
            for comp in comps[::-1]:  # apply the filters in the reverse order
                if comp == "":
                    ...
                    raise ValueError(
                        "Decompression type " + comp + " is not a recognized operation"
                    )

...
def gnomplot(map_, lon, lat, auto=None, sig=0, size=20, vmin=None, vmax=None, rng=None, title=None, ltitle=None, unit=None, cmap="planck", graticule=False, logscale=False, colorbar=True, fwhm=0.0, remove_dip=False, remove_mono=False, fontsize=11, figsize=(5,6), darkmode=False, fignum=None, subplot=None, hold=False, reuse_axes=False, ):
    """
    ...
    if params["logscale"]: 
        m, (vmin, vmax) = apply_logscale(m, [vmin,vmax], linthresh=1)
    ...
    plt.xticks([])
    plt.yticks([])
    plt.text(0.95,0.95, params["title"], color="black", va="top", ha="right", transform=ax.transAxes, fontsize=fontsize+4)
    plt.text(0.05,0.95, params["left_title"], color="black", va="top", transform=ax.transAxes, fontsize=fontsize+4)
    if colorbar:
        # colorbar

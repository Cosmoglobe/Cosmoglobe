...
def Spectrum(pol, long, darkmode, png, foregrounds, masks, nside, cmap="Plotly"):
    ...
    if long:
        ...
        fig, (ax2, ax) = plt.subplots(
            2, 1, sharex=True, figsize=(w, h), gridspec_kw={"height_ratios": [1, ratio]}
        )
    ...
    for fg in foregrounds.keys():
        if not fg == "Sum fg.":
            ...
            foregrounds[fg]["spectrum_mean"] = np.mean(
                foregrounds[fg]["spectrum"], axis=0
            )
            if (
                add_error
                and foregrounds[fg]["spectrum"].shape[0] > 1
                and not fg.startswith("CO")
            ):
                thresh = 0.1
                alpha = 0.5
                foregrounds[fg]["spectrum"][0] = foregrounds[fg]["spectrum"][0] * (
                    1 - np.exp(-(abs(foregrounds[fg]["spectrum"][0] / thresh) ** alpha))
                )
                foregrounds[fg]["spectrum"][1] = foregrounds[fg]["spectrum"][1] / (
                    1 - np.exp(-(abs(foregrounds[fg]["spectrum"][1] / thresh) ** alpha))
                )
            ...
            if i == 0:
                if foregrounds[fg]["spectrum"].shape[0] == 1:
                    ...
                    foregrounds["Sum fg."]["spectrum"] = np.concatenate(
                        (foregrounds[fg]["spectrum"], foregrounds[fg]["spectrum"])
                    ).copy()
                else:
    ...
    for label, fg in foregrounds.items():  # Plot all fgs except sumf
        ...
        if fg["gradient"]:
            ...
            gradient_fill_between(
                ax, nu, fg["spectrum"][k] * 1e-2, fg["spectrum"][k], color=fg["color"]
            )
        else:
            if label == "Sum fg.":
                ax.loglog(
                ...
                if fg["spectrum"].shape[0] > 1:
                    ax.loglog(
                        [lfreq, lfreq],
                        [max(fg["spectrum"][0]), max(fg["spectrum"][1])],
                        linestyle=fg["linestyle"],
                        linewidth=4,
                        color=fg["color"],
                        zorder=1000,
        ...
        if label == "Thermal Dust" and fg["spectrum"].shape[0] > 1:
            ...
            ax.annotate(
                ...
                path_effects=[
                    path_effects.withSimplePatchShadow(alpha=0.8, offset=(0.5, -0.5)),
                ],
            )
            ax.annotate(
    ...
    databands = {
        ...
        "CHI-PASS": {
            "CHI-PASS\n1.4": {
                "pol": False,
                "show": chipass,
                "position": [1.35, ymin * yscaletextup],
        ...
        "QUIJOTE": {
            "11": {
                "pol": True,
                "show": quijote,
                "position": [10.5, ymax2 * yscaletext],
                "range": [10.0, 12.0],
                "color": red,
            },
            "13": {
                "pol": True,
                "show": quijote,
                "position": [13.5, ymax2 * yscaletext],
                "range": [12.0, 14.0],
                "color": red,
            },
            "17\nQUIJOTE": {
        ...
        "DIRBE": {
            "DIRBE\n1250": {
                "pol": False,
                "show": dirbe,
                "position": [dirbepos, ymin * yscaletextup],
                "range": [1000, 1540],
                "color": red,
            },  # DIRBE 1250
            "2140": {
                "pol": False,
                "show": dirbe,
                "position": [1650, ymin * yscaletextup],
                "range": [1780, 2500],
                "color": red,
            },  # DIRBE 2140
            "3000": {
    ...
    bands2 = {
        "Planck": [30, 44, 70, 100, 143, 216, 353, 545, 857],
        "WMAP": [23, 33, 41, 61, 94],
        "S-PASS": [2.303],
        "C-BASS": [5.0],
        "CHI-PASS": [1.4],
        ...
        for experiment, bands in databands.items():
            for label, band in bands.items():
                if band["show"]:
                    ...
                    va = (
                        "bottom" if experiment in bottombands else "top"
...
def getspec(nu, fg, params, function, field, nside, npix, idx, m):
    ...
    # print(fg)
    # Alternative 1
    if any([str(x).endswith(".fits") for x in params]) or any(
        [isinstance(x, np.ndarray) for x in params]
    ):
        if fg == "Spinning Dust":
        ...
        for i, nu_ in enumerate(tqdm(nu, desc=fg, ncols=80)):
            if fg == "Spinning Dust":
                map_[i, idx] = getattr(tls, function)(
                    nu_, *params[:, idx], fnu, f_
                )  # fgs.fg(nu, *params[pix])
            else:
                map_[i, idx] = getattr(tls, function)(
                    nu_, *params[:, idx]
                )  # fgs.fg(nu, *params[pix])

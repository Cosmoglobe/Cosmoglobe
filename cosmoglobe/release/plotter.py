...
def trygveplot(
    ...
    for i, maps in enumerate(maps_):
        ...
        for (
            ...
            m = hp.ma(maps[pl])
            signal_label = (
                signal_labels[polt] if signal_labels else get_signallabel(polt)
            )
            nsid = hp.get_nside(m)
            ...
            if float(fwhm) > 0:  # and filename.endswith(".fits"):
                click.echo(
                    click.style(
                        f"Smoothing fits map to {fwhm} arcmin fwhm", fg="yellow"
                    )
                )
                m = hp.smoothing(
                    m,
            ...
            if nside is not None:  # and filename.endswith(".fits"):
                if nsid != nside:
                    click.echo(
                        click.style(
                            f"UDgrading map from {nsid} to {nside}", fg="yellow"
                        )
                    )
                    m = hp.ud_grade(
                        m,
            ...
            if scale:
                ...
                if "chisq" in datastring:
                    click.echo(
                        click.style(f"Scaling chisq data with dof={scale}", fg="yellow")
                    )
            ...
            #### Automatic variables #####
            if auto:
                (m, ttl, lttl, unt, ticks, cmp, lgscale,) = get_params(
                    m,
                    outfile,
                    outname,
                    signal_label,
            ...
            #### Mask ##########
            grid_map, cmap_ = (
                apply_mask(m, mask, grid_pix, mfill, polt, cmap_)
                if mask
                else (m[grid_pix], cmap_)
            )
...
def get_params(
    ...
    for label, comp in params.items():
        if tag_lookup(comp["tags"], datastring):
            if label == "freefree" and tag_lookup(["diff"], datastring):
                continue
            click.echo(
                click.style(
                    "{:-^48}".format(f"Detected {label} signal {sl} in {datastring}"),
                    fg="yellow",
                )
            )
            ...
            if comp["ticks"] == "auto":
                comp["ticks"] = get_percentile(m, 97.5)
            if label == "chisq":
                ttl = r"$\chi^2$"
            if label == "bpcorr":
                ttl = "$s_{\mathrm{leak}}^{" + tit.lstrip("0") + "}}$"
            if comp["unit"]:
                comp["unit"] = r"$" + comp["unit"].replace("$", "") + "$"
            return (
                m,
                ttl,
                lttl,
    ...
    click.echo(
        click.style(
            "{:-^48}".format(f"Map not recognized, plotting with min and max values"),
            fg="yellow",
        )
    )
...
def apply_logscale(m, ticks, linthresh=1):
    click.echo(
        click.style("Applying semi-logscale", fg="yellow", blink=True, bold=True)
    )
    if linthresh == 0:
        inds = m > 0
...
def get_cmap(cmap, cmp, logscale=False):
    ...
    if "planck" in cmap:
        ...
        try:
            ...
            click.echo(
                click.style("Using qualitative colormap:", fg="yellow")
                + f" {clab} up to {numvals[0]}"
            )
        except:
            cmap = col.ListedColormap(colors, clab)
            click.echo(
                click.style("Using qualitative colormap:", fg="yellow") + f" {clab}"
            )
    ...
    for input_ in input:
        if isinstance(input_, np.ndarray):
            ...
            if dataset.endswith("alm"):
                ...
                (maps_, _, _, _, outfile,) = alm2fits_tool(
                    input_,
                    dataset,
                    nside,
                    lmax,
...
def apply_colorbar(
    fig, image, ticks, ticklabels, unit, fontsize, linthresh, logscale, symlogscale
):
...
def output_map(
    ...
    if gif:
        ...
        ani = animation.ArtistAnimation(
            fig, imgs, interval=interval, blit=True, repeat_delay=1000
        )

...
@click.option("-thinning", default=1, help="thinning factor (default=1)",)
...
def fits_mean(
        input, output, min, max, minchain, maxchain, thinning, chaindir, fwhm, nside, zerospin, missing, pixweight):
    """
    ...
    fits_handler(input, min, max, minchain, maxchain, thinning, chaindir, output, fwhm, nside, zerospin, missing, pixweight, np.mean, write=True)
...
def fits_stddev(
    ...
    fits_handler(input, min, max, minchain, maxchain, thinning, chaindir, output, fwhm, nside, zerospin, missing, pixweight, np.std, write=True)

...
@click.argument("input", type=click.STRING)
@click.option(
    "-mask", type=click.STRING, help="Mask for dipole removal, 30deg sky cut by default"
)
@click.option(
    "-sig",
    type=click.INT,
    multiple=True,
    help="fields to calculate, [0,1,2 by default]",
)
...
def fittemp(input, template, mask, noise, res):
    ...
    if noise:  # noise
        ...
        N = np.eye(2 * npix)
    print(
        "map:", map_.shape, "template:", temp.shape, "mask:", mask.shape, "N:", N.shape
    )
...
@click.option(
    "-beam1",
    type=click.STRING,
    help="Optional beam file for input 1",
)
@click.option(
    "-beam2",
    type=click.STRING,
    help="Optional beam file for input 2",
)
@click.option(
    "-mask",
    type=click.STRING,
...
@click.option(
    "-minchain",
    default=1,
    help="lowest chain number, c0002 [ex. 2] (default=1)",
)
@click.option(
    "-maxchain",
    default=1,
    help="max number of chains c0005 [ex. 5] (default=1)",
)
@click.option(
    "-thinning",
    default=1,
...
@click.option(
    ...
    default=None,
    type=click.STRING,
    help="Base of chain directory, overwrites chain iteration from input file name to iteration over chain directories, BP_chain_c15 to BP_chain_c19 [ex. 'BP_chain', with minchain = 15 and maxchain = 19]",
)
...
@click.option(
    ...
    default=None,
    type=click.INT,
    help="Nside for down-grading maps before calculation",
)
@click.option(
    "-zerospin",
    is_flag=True,
...
@click.option(
    "-missing",
    is_flag=True,
    help="If files are missing, drop them. Else, exit computation",
)
@click.option(
    "-pixweight",
    default=None,

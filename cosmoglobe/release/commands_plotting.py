...
@click.option(
    "-cmap",
    default="Plotly",
    help="set colormap from plotly qualitative colors",
    type=click.STRING,
)
...
def specplot(input, cmap, long, lambdacdm, min_, max_, lmax):
    ...
    ell, ee, bb, eb = np.loadtxt(
        input, usecols=(0, 2, 3, 6), skiprows=3, max_rows=lmax, unpack=True
    )
    ...
    if long:
        f, (ax1, ax2) = plt.subplots(
            2, 1, figsize=(24, 6), gridspec_kw={"height_ratios": [3, 1]}, sharex=True
        )
    else:
        if ratio:
            f, (ax1, ax2) = plt.subplots(
    ...
    if lambdacdm:
        ...
        ax1.loglog(
            ell,
            scaling_TT[0] * TT_lcdm,
            linewidth=2,
            ls=":",
            label=r"TT $\lambda$CDM" + f" - scaled by " + fmt(scaling_TT[0], 1),
        )
        ax1.loglog(
            ell,
            scaling_EE[0] * EE_lcdm,
            linewidth=2,
            ls=":",
            label=r"EE $\lambda$CDM" + f" - scaled by " + fmt(scaling_EE[0], 1),
        )
        ax1.loglog(
            ell,
            scaling_TE[0] * np.abs(TE_lcdm),
            linewidth=2,
            ls=":",
            label=r"TE $\lambda$CDM" + f" - scaled by " + fmt(scaling_TE[0], 1),
        )
        ax1.loglog(
            ell,
            abs(scaling_TE[0] * TE_lcdm + scaling_TT[0] * TT_lcdm),
            linewidth=2,
            ls=":",
            label=r"TT+TE $\lambda$CDM",
        )
...
@click.option(
    "-auto",
    is_flag=True,
    help="Automatically sets all plotting parameters.",
)
@click.option(
    "-min",
    default=None,
...
@click.option(
    "-mid",
    multiple=True,
    help='Adds tick values "-mid 2 -mid 4"',
)
@click.option(
    "-range",
    default="auto",
    type=click.STRING,
    help='Color range. "-range auto" sets to 97.5 percentile of data., or "minmax" which sets to data min and max values.',
...
@click.option(
    ...
    "logscale",
    default=None,
    help="Plots using planck semi-logscale (Linear between -1,1). Autodetector sometimes uses this.",
)
@click.option(
    "-size",
    default="m",
    type=click.STRING,
    help="Size: 1/3, 1/2 and full page width (8.8/12/18cm) [ex. x, s, m or l, or ex. slm for all], m by default",
)
@click.option(
    "-white_background",
    is_flag=True,
...
@click.option(
    "-gif",
    is_flag=True,
    help="Make gifs from input",
)
@click.option(
    "-oldfont",
    is_flag=True,
    help="Use the old DejaVu font and not Times",
)
@click.option(
    "-fontsize",
    default=11,
...
@click.option(
    ...
    default=2000,
    type=click.INT,
    help="figuresize in px (2000 default)",
)
@click.option(
    "-hires",
    is_flag=True,
    help="sets dpi to 3000 and xsize to 10000",
)
...
@click.option(
    "-outname",
    help="Output filename, else, filename with different format.",
)
def gnomplot(
    filename,
    lon,
    ...
    if float(fwhm) > 0:
        click.echo(
            click.style(f"Smoothing fits map to {fwhm} arcmin fwhm", fg="yellow")
        )
        x = hp.smoothing(
            x,
    ...
    if remove_dipole or remove_monopole:
        x = remove_md(x, remove_dipole, remove_monopole, nside)
    ...
    if not outname:
        outname = filename.replace(
            ".fits", f"_gnomonic_{lon}lon{lat}lat_{size}x{size}deg.pdf"
        )
...
@click.option(
    "-mask",
    type=click.Path(exists=True),
    help="Mask for calculating cmb",
)
@click.option(
    "-defaultmask",
    is_flag=True,
    help="Use default dx12 mask",
)
@click.option(
    "-freqmaps",
    is_flag=True,
...
def plotrelease(
    ...
    if all_:
        ...
        goodness_temp = not goodness_temp
        goodness_pol = not goodness_pol
        chisq = not chisq
    ...
    for colorbar in [True, False]:
        if (cmbresamp and mask) or (cmbresamp and defaultmask):
            ...
            try:
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_c0001_Tresamp_{procver}.h5",
                    dataset="cmb/amp_alm",
                    nside=1024,
                    fwhm=14,
                    lmax=2000,
            ...
            try:
                ctx.invoke(
                    plot,
                    input=f"CG_c0001_Presamp_{procver}.h5",
                    dataset="cmb/amp_alm",
                    nside=1024,
                    fwhm=14,
                    lmax=2000,
        ...
        if (cmb and mask) or (cmb and defaultmask):
            ...
            try:
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_cmb_IQU_n1024_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
                ...
                ctx.invoke(
                    ...
                    auto=True,
                    remove_dipole=mask,
                    fwhm=np.sqrt(420.0**2 - 14**2),
                    range=150,
                )

                # CMB QU at 14 arcmin, 1 degree and 7 degree smoothing
                for i, fwhm in enumerate(
                    [0.0, np.sqrt(60.0**2 - 14**2), np.sqrt(420.0**2 - 14**2)]
                ):
            ...
            try:
                ...
                for j in ["a", "b", ""]:
                    for i in range(1, 11):
                        ctx.invoke(
                            plot,
                            input=f"CG_DIRBE_{i:02}{j}_I_n0512_{procver}.fits",
                            outdir=outdir,
                            colorbar=colorbar,
                            auto=True,
                            size=size,
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_030_IQU_n0512_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_044_IQU_n0512_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_070_IQU_n1024_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
                ...
                rms_lims = [150, 120, 160, 150, 220, 200, 350, 370, 400, 380]
                for rms, lab in zip(rms_lims, bandlabs):
                    ctx.invoke(
                        plot,
                        input=f"CG_{lab}_IQU_n0512_{procver}.fits",
                        size=size,
                        outdir=outdir,
                        colorbar=colorbar,
                        auto=True,
        ...
        if synch:
            ...
            try:
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_synch_IQU_n1024_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
        ...
        if co:
            ...
            try:
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_CO_tot_I_n1024_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
        ...
        if stars:
            ...
            try:
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_stars_I_n0512_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
        ...
        if cii:
            ...
            try:
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_cii_I_n1024_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
        ...
        if hotpah:
            ...
            try:
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_hotPAH_I_n1024_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
        ...
        if ff:
            ...
            try:
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_freefree_I_n1024_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
        ...
        if ame:
            ...
            try:
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_ame_I_n1024_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
        ...
        if dust:
            ...
            try:
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_dust_I_n2048_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
            ...
            try:
                ...
                ctx.invoke(
                    plot,
                    input=f"CG_dust_IQU_n1024_{procver}.fits",
                    size=size,
                    outdir=outdir,
                    colorbar=colorbar,
                    auto=True,
        ...
        if goodness:
            ...
            if goodness_pol:
                ...
                masks = [
                    "wmap_processing_mask_Ka_r4_9yr_v5_TQU_chisq50.fits",
                    "wmap_processing_mask_Q_r4_9yr_v5_TQU_chisq50.fits",
                    "wmap_processing_mask_V_r4_9yr_v5_TQU_chisq50.fits",
                ]
                m = 0
                for band in pbands:
                    try:
                        if (band in ["030_IQU", "044_IQU", "070_IQU"]) or (
                            "WMAP" in band
                        ):
                            sig = [1, 2, 4, 5]
                        else:
            ...
            if chisq:
                ...
                scale = (
                    fudge
                    * 2
                    * (np.sum([(x / 16) ** 2 for x in nsides]) - 3 * (64 / 16) ** 2)
                )
                ctx.invoke(
                    plot,
                    input=fname,
...
@click.option(
    ...
    multiple=True,
    help="This is regions for synch",
)
@click.option(
    "-prior", default=[], nargs=2, multiple=True, help="Specify mean and stddev"
)
...
def hist(chainfile, dataset, burnin, maxchain, lloc, nbins, sig, idx, prior, min, max):
    ...
    if "synch" in dataset:
        ...
        if prior != []:
            for i, pri in enumerate(prior):
                ...
                plt.plot(
                    ...
                    color=f"k",
                    linestyle=":",
                    label=r"$P(\beta_{\mathrm{s}})=\mathcal{N}("
                    + f"{pri[0]}, {pri[1]}"
                    + ")$",
                )
        ...
        if prior != []:
            for pri in prior:
                ...
                plt.plot(
                    ...
                    color="black",
                    linestyle=":",
                    label=r"$P(\beta_{\mathrm{d}})=\mathcal{N}("
                    + f"{pri[0]}, {pri[1]}"
                    + ")$",
                )
        ...
        n1, bins, _ = plt.hist(
            x[:, 1], bins=nbins, histtype="step", density=True, stacked=True
        )
        ymax = np.max(n1)
        for i in range(x.shape[1]):
            (nn, _, _,) = plt.hist(
                x[:, i], bins=bins, histtype="step", density=True, stacked=True
            )
            if np.max(nn) > ymax:
                ymax = np.max(nn)
        ...
        if prior != []:
            for pri in prior:
                ...
                plt.plot(
                    ...
                    color=f"C{i}",
                    linestyle=":",
                    label=r"$\mathcal{N}(" + f"{pri[0]}, {pri[1]}" + ")$",
                )
    ...
    plt.tight_layout()
    plt.savefig(
        dataset.replace("/", "-") + "_histogram.pdf",
        dpi=300,
        bbox_inches="tight",
        pad_inches=0.02,
    )
...
@click.option(
    "-cmap", default="Plotly", help="Sets colorcycler using plotly", type=click.STRING
)
def traceplot(filename, max, min, nbins, cmap):
    ...
    traceplotter(
        ...
        x,
        nbins,
        outname=filename.replace(".dat", "_traceplot.pdf"),
        min_=min,
        cmap=cmap,
    )
...
@click.option(
    ...
    "priorsamp",
    multiple=True,
    help="These are sampled around prior and will be marked",
)
...
def pixreg2trace(
    chainfile, dataset, burnin, maxchain, plot, freeze, nbins, priorsamp, scale, cmap
):
    """
    ...
    if "bp_delta" in dataset or "bandpass" in dataset:
        ...
        if nregs == 9:
            ...
            header = ["High lat.", "Spur", "Center", "Fan", "Anti-center", "Gum nebula"]
        elif nregs == 4:
            header = ["High lat.", "Spur", "Gal. center", "Gal. plane"]
        elif nregs == 1:
            header = [
                "Fullsky",
            ]
    ...
    for comp in comps:
        ...
        for dirtype, dirloc in zip([type1, type2], [dir1, dir2]):
            ...
            if len(filenames[dirloc]) == 0:
                mapn[dirloc] = glob.glob(
                    os.path.join(dirloc, "CG_" + comp + "_I*.fits")
                )[0]
            else:
...
def output_sky_model(
    ...
    if pol:
        ...
        foregrounds = {
            ...
            r"CO$_{2\rightarrow 1}$": {
                "function": "line",
                "params": [a_co21, 230.0, 14.01],
                "position": p,
                "color": "C9",
                "sum": True,
                "linestyle": "solid",
            ...
            r"CO$_{3\rightarrow 2}$": {
                "function": "line",
                "params": [a_co32, 345.0, 12.24],
                "position": p,
                "color": "C9",
                "sum": True,
                "linestyle": "solid",

...
from cosmoglobe.refactored_sky.protocols import SkyComponent
from cosmoglobe.refactored_sky.constants import DEFAULT_OUTPUT_UNIT, NO_SMOOTHING
...
class SkySimulator:
    ...
    def __call__(
        ...
        diffuse_comps: List[SkyComponent] = []
        pointsource_comps: List[SkyComponent] = []
        for comp in components:
            if comp.component_type is SkyComponentType.POINTSOURCE:
                pointsource_comps.append(comp)
            else:
                diffuse_comps.append(comp)
